...
const sayHelloBackgroundback = (event) => {
    sayHelloButton.classList.add('sayHelloBackground');
}
...
// CODE HERE
const sayHelloBackgroundBack = () => {
    sayHelloButton.classList.remove('sayHelloBackground');
}

sayHelloButton.addEventListener('mouseout', sayHelloBackground);
...
const repeatMyParam = () => {
    axios.get('http://localhost:3000/repeat/Ibrahim')
        .then(res => {
            let p = document.querySelector('#repeat-text');
...
// CODE HERE
const query = () => {
    axios.get('http://localhost:3000/query-test/?animals=5435f/?ggr=nrgoinr')
    .then(res => {
...
/*
    ...
    Console logged: "you sent more than 1 querry"
*/
...
const createFood =(event) => {
    ...
    let foodInput = document.querySelector('#inputField');
    let body = {
